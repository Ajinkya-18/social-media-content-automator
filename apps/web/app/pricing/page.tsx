  ...
  const handleBuy = async (planType: string) => {
    ...
    try {
      ...
      const options = {
        ...
        theme: {
          color: "#2563eb", // Changed from Orange to Blue-600
        },
      };
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 animate-in fade-in duration-500">
      ...
      <div className="text-center mb-16">
        <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
          Fuel Your <span className="text-cyan-400 drop-shadow-[0_0_15px_rgba(34,211,238,0.5)]">Creativity</span>
        </h1>
        ...
        {isSignedIn && (
            <div className="mt-8 inline-flex items-center gap-2 px-4 py-2 bg-blue-900/20 rounded-full border border-blue-500/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto mb-20">
        ...
        {/* PRO (Highlighted - Blue/Purple) */}
        <div className="relative transform md:-translate-y-4">
            <div className="absolute inset-0 bg-gradient-to-b from-blue-600 to-purple-600 rounded-2xl blur-lg opacity-40" />
            <PricingCard 
                title="Pro" 
                price="₹1500" 
                period="/month"
      ...
      <div className="text-center border-t border-white/5 pt-12">
        <p className="text-slate-500 text-sm">
            ...
            Need a custom enterprise plan? <a href="#" className="text-cyan-400 hover:underline">Contact Sales</a>
        </p>
      </div>
...
function PricingCard({ title, price, period, credits, features, icon, highlighted = false, onBuy, loading, buttonText = "Choose Plan" }: any) {
    return (
        <div className={`relative h-full flex flex-col p-8 rounded-2xl border ${highlighted ? 'bg-[#0f172a] border-blue-500 shadow-2xl shadow-blue-900/20' : 'bg-white/5 border-white/10'} transition-all hover:scale-105 duration-300`}>
            
            <div className="mb-6">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${highlighted ? 'bg-blue-500/20' : 'bg-white/5'}`}>
                    {icon}
                </div>
                ...
                <div className="mt-4 px-3 py-1.5 bg-white/5 rounded-lg inline-block border border-white/5">
                    <span className="text-cyan-300 font-bold text-sm">💎 {credits}</span>
                </div>
            </div>
            ...
            <ul className="space-y-4 mb-8 flex-1">
                {features.map((feat: string, i: number) => (
                    <li key={i} className="flex items-start gap-3 text-slate-300 text-sm">
                        <Check className={`w-4 h-4 mt-0.5 ${highlighted ? 'text-blue-400' : 'text-slate-500'}`} />
                        {feat}
                    </li>
                ))}
            </ul>
